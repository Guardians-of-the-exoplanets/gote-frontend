...
const researcherSteps: TutorialStep[] = [
  ...
  },
  {
    ...
    description:
      "Our production pipeline uses XGBoost with stratified K-fold cross-validation, rigorous dataset splitting, and feature selection to ensure reproducible and scientifically valid classifications.",
    details: [
      "Dataset Split: 70% Training / 20% Test / 10% Blind Validation",
      "Overfitting Prevention: StratifiedKFold (K=5) with separate test partition evaluation",
      "Base Model: XGBClassifier with eval_metric='mlogloss' and objective='multi:softprob'",
      "Tuned Model (when hyperparameters enabled): colsample_bytree=0.8, learning_rate=0.1, max_depth=6, n_estimators=100, subsample=0.8",
      "Feature Selection: Analysis of 10 most important features with threshold comparison at 90%, 95%, and 99%",
      "Evaluation: Test partition and blind validation metrics (Accuracy, Precision, Recall, F1-Score, Macro/Weighted-Avg)",
      "Grid Search CV: Applied when appropriate to optimize hyperparameters",
    ],
    inputs: [
      { name: "Hyperparameter Switch", description: "Toggle between baseline model (version 1) and tuned model (version 2 with custom hyperparameters)" },
      { name: "colsample_bytree", description: "Subsample ratio of columns when constructing each tree (0.8 = 80% of features)" },
      { name: "learning_rate", description: "Step size shrinkage to prevent overfitting (0.1)" },
      { name: "max_depth", description: "Maximum depth of a tree, controls model complexity (6)" },
      { name: "n_estimators", description: "Number of boosting rounds / trees in the ensemble (100)" },
      { name: "subsample", description: "Subsample ratio of training instances (0.8 = 80% of data per tree)" },
    ],
    outputs: [
      { name: "Test Metrics", description: "Evaluation on 20% test partition: Accuracy, F1-Score, Precision, Recall" },
      { name: "Blind Validation Metrics", description: "Evaluation on 10% holdout set to validate generalization" },
      { name: "K-Fold Cross-Validation", description: "5-fold stratified results showing model stability across splits" },
      { name: "Feature Importance", description: "Top 10 most influential features identified during training" },
      { name: "Classification Report", description: "Per-class metrics and macro/weighted averages" },
      { name: "Confusion Matrix", description: "True vs predicted labels for test and blind validation sets" },
    ],
    tips: [
      "Baseline model (version 1) provides consistent results for comparison across experiments",
      "Tuned model (version 2) uses hyperparameters optimized via Grid Search CV for better performance",
      "Stratified K-Fold ensures each fold maintains the same class distribution as the full dataset",
      "Blind validation set is never seen during training or hyperparameter tuning, ensuring unbiased evaluation",
      "Feature thresholds (90%, 95%, 99%) help identify the minimum feature set needed for optimal performance",
    ],
    icon: Settings,
  },
  {
