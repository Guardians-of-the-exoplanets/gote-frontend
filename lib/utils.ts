...
export function normalizeId(row: RawRow): string {
  return String(row?.id ?? row?.object_id ?? row?.koi ?? row?.ID ?? row?.name ?? '').trim()
}

export function normalizeClassification(row: RawRow): 'Confirmed' | 'Candidate' | 'False Positive' {
  const raw = String(row?.classificacao ?? row?.classification ?? row?.label ?? '').toLowerCase().normalize('NFD').replace(/[^a-z ]/g,'')
...
export function normalizeProbability(row: RawRow): number {
  const probRaw = row?.probabilidade ?? row?.probability ?? row?.prob ?? row?.score ?? null
